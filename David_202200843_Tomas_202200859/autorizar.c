...
int authorizePlane() {
    ...
    if(FlightsVerifier != 1) {
        replacePlaneStr(id, "currentState", "Ready for authorization");
        ...
        printf("\nERROR: Fly limit reached or airplane is not ready to fly.");
    }
    ...
    while (fread(&Airplane, sizeof(typeAirplane), 1, File)) {
        ...
        if (Airplane.id == id) {
            if (strcmp(field, "currentState") == 0) {
                memcpy(Airplane.currentState, newValue, 40);
            }
        }
...
int replacePlaneAddFly(int id, const char *field){
    ...
    while (fread(&Airplane, sizeof(typeAirplane), 1, File)) {
        ...
        if (Airplane.id == id) {
            ...
            if(Airplane.totalFlights >= 100 || Airplane.capacity < 2 || strcmp(Airplane.currentState, "Ready for authorization") != 0){
                valueToReturn = 1;
            }
...
void replacePlaneUpdateLastFlights(int id, const char *field){
    ...
    while (fread(&Airplane, sizeof(typeAirplane), 1, File)) {
        ...
        if (Airplane.id == id) {
            ...
            Airplane.passengerId1 = 0;
            Airplane.passengerId2 = 0;
            Airplane.passengerId3 = 0;
            Airplane.passengerId4 = 0;
            memcpy(Airplane.currentState, "In preparation", 17);
            Airplane.capacity = 0;
        }
